// Jest setup for Selenium screenshot tests
const fs = require('fs');
    ...
    if (isMatch) {
      ...
      return {
        message: () => 
          ...
          `  Diff image: ${diffPath}\n` +
          `  To update baseline, run: npm run e2e:selenium:update-screenshots`,
        pass: false,
      };
    }
  },
...
if (process.env.USE_PERCY === 'true') {
  afterAll(async () => {
    const { finalizePercySelenium } = require('./percy-selenium-adapter');
    await finalizePercySelenium();
  }, 30000);
}
