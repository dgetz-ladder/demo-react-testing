...
const { createDriver, By } = require('../selenium.config');
const { captureScreenshot, captureElementScreenshot, compareDiff, expectMatch } = require('../utils/selenium-screenshots');
...
const load = async () => {
    ...
    await driver.sleep(2000); // Wait for network idle
    
    // Wait for root element to be visible
    await driver.wait(async () => {
        const element = await driver.findElement(By.css('#root'));
